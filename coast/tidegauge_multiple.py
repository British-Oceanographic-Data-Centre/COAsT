...
##############################################################################
    ...
    def obs_operator(self, gridded, time_interp='nearest'):
        ...
        ind2D = general_utils.nearest_indices_2D(gridded.longitude, gridded.latitude, 
                                                 ds.longitude, ds.latitude,
                                                 mask = gridded.landmask)
        
        # Extract spatial time series
        print('Calculating time indices.', flush=True)
        extracted = gridded.isel(x_dim = ind2D[0], y_dim = ind2D[1])
        ...
        # Interpolate model onto obs
        print('Interpolating in time...', flush=True)
        extracted = extracted.interp(time = extracted.time_instant.values, method = time_interp)
