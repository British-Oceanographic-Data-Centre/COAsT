...
##############################################################################
    ...
    def harmonic_analysis_utide(self, min_datapoints=1000, nodal=False, 
        ...
        for pp in range(0,n_port):
            ...
            uts_obs = ut.solve(time, ssh.values, lat=ssh.latitude.values,
                               nodal=nodal, trend=trend, method=method, 
                               conf_int=conf_int, Rayleigh_min=Rayleigh_min)
    ...
    def reconstruct_tide_utide(self, utide_solution_list, constit=None):
        ...
        for pp in np.arange(n_port):    
            ...
            tide = np.array( ut.reconstruct(time, pp_solution, 
                                            constit=constit).h )
    ...
    def calculate_residuals(self, tg_tide, apply_filter=True, 
    ...
    def threshold_statistics(self, thresholds=np.arange(-.4,2,0.1), 
                             peak_separation=12):
    ...
    def obs_operator(self, gridded, time_interp='nearest'):
        ...
        interp_dist = general_utils.calculate_haversine_distance(extracted.longitude, 
                                                                 extracted.latitude, 
                                                                 ds.longitude.values,
                                                                 ds.latitude.values)
