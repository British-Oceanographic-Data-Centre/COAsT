...
class INTERNALTIDE(NEMO):
    ...
    def __init__(
        self, nemo_t: xr.Dataset, nemo_w: xr.Dataset
    ):  # TODO We're not calling super init...
    ...
    def construct_pycnocline_vars(
        self, nemo_t: xr.Dataset, nemo_w: xr.Dataset, strat_thres=-0.01
    ):
        """
        ...
        strat_m = (
            nemo_w.dataset.latitude * 0 + 1
        )  # create a stratification mask: [1/0] = strat/un-strat
        strat_m = strat_m.where(
            strat.min(dim="z_dim").squeeze() < strat_thres, 0, drop=False
        )
        ...
        intz2N2 = (xr.ufuncs.square(depth_0_4d - zd) * e3_0_4d * strat).sum(
        ...
        coords = {
            "time": (("t_dim"), nemo_t.dataset.time.values),
        ...
        self.dataset.strat_2nd_mom.attrs[
            "long_name"
        ] = "Second depth moment of stratification"

        self.dataset["strat_1st_mom"] = xr.DataArray(zd, coords=coords, dims=dims)
        self.dataset.strat_1st_mom.attrs["units"] = "m"
        self.dataset.strat_1st_mom.attrs["standard_name"] = "pycnocline depth"
        self.dataset.strat_1st_mom.attrs[
            "long_name"
        ] = "First depth moment of stratification"
