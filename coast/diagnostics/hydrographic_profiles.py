...
###############################################################################
 def __init__ (self,filename='none',datasetnames='none',config='',regionbounds=[],recalc=False):
    #Reads and manipulates lists of hydrographic profiles
    #if called with datasetnames and regionbounds, extract profiles in these bounds 
 ...
 #    data_size=400
     ...
     self.profile = self.profile.subset_indices_lonlat_box(lonbounds = [x_min, x_max], 
                                            latbounds = [y_min, y_max])
...
######################################################################################## 
 def saveprofiles(self,filename):
     ...
     with ProgressBar():
        self.profile.dataset.to_netcdf(filename_gridded)  
...
#Match profiles locations to grid
#optionally provide region of grid to sub-set [jmin,jmax,imin,imax]
...
#Sort 4 NN by distance on grid
     ...
     rr=Hydrographic_Profiles.distance_on_grid(lat_grd,lon_grd,j_prf[ip,:].ravel(),i_prf[ip,:].ravel(),lat_prf4,lon_prf4)
...
#fix memory issues for very large data sets, if this still nbeeded with xarray?
     if nprof < 1000000:
      npr=nprof
     else:
      npr = int(nprof/10)
     ...
     for ichnk in Hydrographic_Profiles.chunks(range(0,nprof),npr):
        ...
        #define interface layers and DZ associated with Z
         Zw=np.empty_like(ZZ);DZ=np.empty_like(ZZ)
         Zw[:,0]=0.
         I=np.arange(0,nz-1)
        ...
        # depth from model
         ...
         kbot=np.ones((nprof), dtype=int)*np.nan
         T=np.zeros(nz)*np.nan;S=np.zeros(nz)*np.nan;Z=np.zeros(nz)*np.nan;ZW=np.zeros(nz)*np.nan
         DP[DP==0]=np.nan
        ...
        #find good profiles
           ...
           for iq in range(len(quart)-1):
            I=np.nonzero(np.all(np.concatenate(([Z<=-DD*quart[iq]],[Z>=-DD*quart[iq+1]]),axis=0),axis=0))
...
###############################################################################   
 ...
 def nn_search(X,Y,Xpnts,Ypnts,n_nn): #redundent - can delete
    #efficient nearest neighbour search    
     import scipy.spatial as sp   
     XY=np.dstack([Y.ravel(),X.ravel()])[0]
     XYp=np.dstack([Ypnts,Xpnts])[0]   
     mytree=sp.cKDTree(XY)
     dist,indx=mytree.query(XYp,n_nn)
     I=np.nonzero(np.isnan(Xpnts))
     indx[I]=0
     i,j=np.unravel_index(indx,X.shape)
     return i,j,dist
###########################################
 def distance_on_grid(Y,X,jpts,ipts,Ypts,Xpts):
       import numpy as np
       DX=(Xpts-X[jpts,ipts])*Re*np.cos(Ypts*np.pi/180.)
