...
class CurrentsonT(Gridded):
    ...
    def currents_on_T(self,gridded_u,gridded_v):
        ...
        UT[:,:,:,1:]=0.5*(gridded_u.dataset.u_velocity.values[:,:,:,1:]
                         +gridded_u.dataset.u_velocity.values[:,:,:,:-1])
        #V velocity on T points
        VT=np.zeros(gridded_v.dataset.v_velocity.shape)
        VT[:,:,1:,:]=0.5*(gridded_v.dataset.v_velocity.values[:,:,1:,:]
                         +gridded_v.dataset.v_velocity.values[:,:,:-1,:])
    ...
    def plot_surface_circulation(self,name
                                  ,Vmax=0.16,Np=3
