...
# does so in time blocks (multiples of months) to avoid memory problems. 
...
# This is the index number of this process. Passed as command line argument
...
print("Analysis Range: {0} -->> {1}".format(start_date.strftime('%Y%m%d'), 
                                            end_date.strftime('%Y%m%d')), flush=True)
...
nemo.dataset = nemo.dataset[["temperature","salinity","bathymetry",
                             "bottom_level","landmask"]]
profile.dataset = profile.dataset[['potential_temperature',
                                   'practical_salinity','depth']]
...
model_profiles_interp = profile_analysis.interpolate_vertical(model_profiles, profile, 
...
model_profiles.dataset.to_netcdf(dn_out+"extracted_profiles_{0}_{1}_{2}.nc".format(run_name, 
                                 start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))
model_profiles_interp.dataset.to_netcdf(dn_out + "interpolated_profiles_{0}_{1}_{2}.nc".format(run_name,
                                        start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))
...
profile_interp.dataset.to_netcdf(dn_out + "interpolated_obs_{0}_{1}_{2}.nc".format(run_name, 
                                 start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))
...
differences.dataset.to_netcdf(dn_out+"profile_errors_{0}_{1}_{2}.nc".format(run_name,
                              start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))
...
surface_data = xr.merge((surface_errors.dataset, model_profiles_surface.dataset, 
                         obs_profiles_surface.dataset), compat='override')
surface_data.to_netcdf(dn_out+"surface_data_{0}_{1}_{2}.nc".format(run_name,
                       start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))

# Bottom values and errors
model_profiles_bottom = profile_analysis.bottom_means(model_profiles, 
                                                      bottom_height, bottom_thresh)
...
bottom_data = xr.merge((bottom_errors.dataset, model_profiles_bottom.dataset, 
                        obs_profiles_bottom.dataset),
                          compat="override")
bottom_data.to_netcdf(dn_out+"bottom_data_{0}_{1}_{2}.nc".format(run_name,start_date.strftime('%Y%m'),end_date.strftime('%Y%m')))
