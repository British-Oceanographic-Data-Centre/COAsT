...
class test_process_data_methods(unittest.TestCase):
    def test_statsmodel_seasonal_decompose(self):
        ...
        fake_time = np.arange(
            np.datetime64("2010-01-01"), np.datetime64("2014-01-01"), np.timedelta64(1, 'M'), 
            dtype='datetime64[M]'
        ).astype('datetime64[s]')
        fake_temp = ( 
        ...
        fake_data_array = xr.DataArray( fake_temp, coords={
                                            "t_dim": fake_time,
        ...
        grd = proc_data.seasonal_decomposition(
               fake_data_array, 4, model='additive', period=6, extrapolate_trend="freq"
        )
        # statsmodel version
        from statsmodels.tsa.seasonal import seasonal_decompose
        stm_result = seasonal_decompose(
            fake_data_array.fillna(0).values.reshape( (48,2*375*297) ), model='additive', period=6, extrapolate_trend="freq"
        )

        check_trend = np.allclose( stm_result.trend.reshape((48,2,375,297)), grd.dataset.trend.compute().fillna(0) )
        check_seasonal = np.allclose( stm_result.seasonal.reshape((48,2,375,297)), grd.dataset.seasonal.compute().fillna(0) )
        check_residual = np.allclose( stm_result.resid.reshape((48,2,375,297)), grd.dataset.residual.compute().fillna(0) )
