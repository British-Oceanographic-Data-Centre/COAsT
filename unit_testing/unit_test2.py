...
try:
    sci = coast.Gridded(path.join(dn_files, fn_nemo_dat),
                        path.join(dn_files, fn_nemo_dom), config=json_config_file_t_grid)
...
try:
    ...
    sci_load_ds = coast.Gridded(config=json_config_file_t_grid)
    sci_load_ds.load_dataset(ds)
    sci_load_file = coast.Gridded(config=json_config_file_t_grid)
    ...
    if sci_load_ds.dataset.identical(sci_load_file.dataset):
        ...
        print(str(sec) + chr(
            subsec) + " X - COAsT.load_dataset() ERROR - not identical to dataset loaded via COAsT.load()")
except:
    print(str(sec) + chr(subsec) + " FAILED")
...
pass_test = False
nemo_f = coast.Gridded(fn_domain=dn_files + fn_nemo_dom, config=json_config_file_f_grid)
...
try:
    nemo_t = coast.Gridded(fn_data=dn_files + fn_nemo_grid_t_dat,
                        fn_domain=dn_files + fn_nemo_dom, config=json_config_file_t_grid)
...
try:
    ...
    amm7 = coast.Gridded(dn_files + fn_nemo_dat_subset,
                      dn_files + fn_nemo_dom, config=json_config_file_t_grid)
...
try:
    ...
    amm7 = coast.Gridded(dn_files + file_names_amm7,
                      dn_files + fn_nemo_dom, config=json_config_file_t_grid, multiple=True)
...
try:
    harmonics = coast.Gridded(dn_files + fn_nemo_harmonics,
                           dn_files + fn_nemo_harmonics_dom, config=json_config_file_t_grid)
...
try:
    ...
    nemo_w_4 = nemo_t.differentiate('depth4D', dim='z_dim', out_varstr='dzdz', config_path=json_config_file_w_grid)
    if not np.isclose(nemo_w_4.dataset.dzdz.isel(z_dim=slice(1, nemo_w_4.dataset.dzdz.sizes['z_dim'])).max(), -1) \
            or not np.isclose(nemo_w_4.dataset.dzdz.isel(z_dim=slice(1, nemo_w_4.dataset.dzdz.sizes['z_dim'])).min(),
                              -1):
...
try:
    ...
    check3 = tg.get_tidetabletimes(np.datetime64('2020-10-13 12:48'), method='nearest_1').time.values == np.datetime64(
        '2020-10-13 14:36')
    check4 = np.array_equal(tg.get_tidetabletimes(np.datetime64('2020-10-13 12:48'), method='nearest_2').values,
                            [2.83, 8.01])
