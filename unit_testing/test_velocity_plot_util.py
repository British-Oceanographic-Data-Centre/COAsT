...
class TestPlotUtilitiesVelocity(unittest.TestCase):
    """
    Tests for plotting utilities using velocity data
    """

    def test_velocity_polar_bug_fix(self):
        ...
        self.assertTrue(np.isclose(u_new, result1).all(), "result1")
        result2 = np.array([0.81649658, 0.70363179, 0.55052735, 0.35434932, 0.12279135])
        self.assertTrue(np.isclose(v_new, result2).all(), "result2")
    ...
    def test_make_projection(self):
        ...
        self.assertTrue(test_proj.prime_meridian.unit_name == "degree", "prime_meridian")
        self.assertTrue(isinstance(test_proj, pyproj.CRS), "test_proj is CRS")
    ...
    def test_velocity_rotate(self):
        ...
        self.assertTrue(np.isclose(u_rotate, 0.93969) & np.isclose(v_rotate, -0.34202), "u_rotate, v_rotate")
    ...
    def test_grid_angle(self):
        ...
        self.assertTrue(np.isclose(angle, 90, rtol=0.5).all(), "angle")
        angle = plot_util.grid_angle(lat, lon)
        self.assertTrue(np.isclose(angle, 0, rtol=0.5).all(), "angle")
    ...
    def test_velocity_on_t(self):
        ...
        self.assertTrue(np.isclose(u_on_t_points, result1).all(), "result1")
        result2 = np.array(([3, 4, 4], [3.5, 4, 3.5], [2.5, 2.5, 2]))
        self.assertTrue(np.isclose(v_on_t_points, result2).all(), "result2")
    ...
    def test_velocity_grid_to_geo(self):
        ...
        u_new, v_new = plot_util.velocity_grid_to_geo(lon, lat, uv_velocity)
        u_result1 = np.array(
            (
        ...
        self.assertTrue(np.isclose(u_new, u_result1).all() & np.isclose(v_new, v_result1).all(), "u_new, v_new")

        u_new, v_new = plot_util.velocity_grid_to_geo(lon, lat, uv_velocity, True)
        u_result2 = np.array(
            (
        ...
        self.assertTrue(np.isclose(u_new, u_result2).all() & np.isclose(v_new, v_result2).all(), "u_new, v_new")

    def test_plot_polar_contour(self):
        ...
        self.assertTrue(isinstance(cs1, cartopy.mpl.contour.GeoContourSet), "cs1")
