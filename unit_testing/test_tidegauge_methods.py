...
class test_tidegauge_methods(unittest.TestCase):
    ...
    def test_tidegauge_resample_and_apply_doodsonx0(self):
        ...
        with self.subTest("Resample to 1H"):
            date0 = datetime.datetime(2007, 1, 10)
            date1 = datetime.datetime(2007, 1, 12)
            lowestoft = coast.Tidegauge(files.fn_tidegauge, date_start=date0, 
                                        date_end=date1)
    ...
    def test_load_multiple_tidegauge(self):
        ...
        with self.subTest("Load multiple gauge"):
            ...
            date1 = datetime.datetime(2007, 1, 12)
            lowestoft = coast.Tidegauge(files.fn_tidegauge, date_start=date0, date_end=date1)
            tidegauge_list = coast.Tidegauge.create_multiple_tidegauge(files.fn_multiple_tidegauge, 
                                                                       date0, date1)
    ...
    def test_tidegauge_finding_extrema(self):
        ...
        with self.subTest("Find extrema"):
            ...
            lowestoft2 = coast.Tidegauge(files.fn_tidegauge, 
        ...
        with self.subTest("Plot extrema"):
            ...
            plt.scatter(extrema_comp.dataset.time_highs.values, 
                        extrema_comp.dataset.sea_level_highs, marker="o", c="g")
            plt.scatter(extrema_comp.dataset.time_lows.values, 
                        extrema_comp.dataset.sea_level_lows, marker="o", c="r")
    ...
    def test_tidegauge_cubic_spline_extrema(self):
        ...
        with self.subTest("Fit cubic spline"):
            ...
            tg.dataset = tg.read_bodc_to_xarray(files.fn_tidegauge2, date_start, 
                                                date_end)
            ...
            # Check actual maximum/minimum is in output dataset
            check1 = np.isclose(extrema_cubc.dataset.sea_level_highs, 
                                [7.77432795, 7.91244559]).all()
            check2 = np.isclose(extrema_cubc.dataset.sea_level_lows, 
                                [2.63479458, 2.54599355]).all()
        ...
        with self.subTest("Attempt plot"):
            ...
            plt.scatter(extrema_cubc.dataset.time_highs.values, 
                        extrema_cubc.dataset.sea_level_highs, marker="o", c="g")
            plt.scatter(extrema_cubc.dataset.time_lows.values, 
                        extrema_cubc.dataset.sea_level_lows, marker="o", c="r")
