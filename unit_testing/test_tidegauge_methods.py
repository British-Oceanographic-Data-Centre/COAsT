...
class test_tidegauge_analysis(unittest.TestCase):
    def test_match_missing_values(self):
        ...
        tg1, tg2 = tganalysis.match_missing_values(lowestoft.dataset.ssh, lowestoft2.dataset.ssh)
    ...
    def test_demean_timeseries(self):
        ...
        #self.assertTrue((np.isclose(tg1.dataset.ssh[0, :].values, demeaned)).all(), "check1")  # might need this with later numpy>1.26 versions
    ...
    def test_harmonic_analysis_utide(self):
        ...
        with self.subTest("Reconstruct time series"):
            ...
            self.assertTrue(
                reconstructed.dataset.time.values[0] == np.datetime64("2008-01-01T00:00:00.000000000"), "check2"
            )
        ...
        with self.subTest("Calculate non-tidal residuals"):
            reconstructed = tganalysis.reconstruct_tide_utide(lowestoft.dataset.ssh, ha)
            ntr = tganalysis.calculate_non_tidal_residuals(lowestoft.dataset.ssh, reconstructed.dataset.reconstructed)
