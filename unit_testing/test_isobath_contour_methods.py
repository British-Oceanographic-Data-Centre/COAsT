...
class test_isobath_contour_methods(unittest.TestCase):
    ...
    def test_calculate_pressure_along_contour(self):
        ...
        check1 = np.allclose(
            (cont_t.data_contour.pressure_s + 
             cont_t.data_contour.pressure_h_zlevels).sum().compute().item(), 
            27490693.20181531
        )
    ...
    def test_calculate_preddure_gradient_griven_flow(self):
        ...
        y_ind, x_ind, contour = coast.Contour.get_contour_segment(nemo_f, contours[0], 
                                                                  [50, -10], [60, 3])
        cont_f = coast.ContourF(nemo_f, y_ind, x_ind, 200)
        cont_f.calc_geostrophic_flow(nemo_t, config_u=files.fn_config_u_grid, 
                                     config_v=files.fn_config_v_grid, ref_density=1027)
        check1 = np.allclose(
            (
